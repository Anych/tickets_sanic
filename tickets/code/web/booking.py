...
async def booking_details(request, booking_id):
    async with request.app.ctx.db_pool.acquire() as conn:
        ...
        offer_details = [{'id': d['id'], 'offer_id': d['offer_id'], 'flights': None, 'refundable': d['refundable'],
                          'baggage': d['baggage'], 'cabin': d['cabin'], 'airline': d['airline'],
                          'passengers': d['passengers'], 'type': d['type']} for d in offer_details][0]
        ...
        for num in range(len(flights_details)):
            segment_details = await conn.fetch(f"SELECT * FROM segment WHERE flight_id = '{flights_details[num]['id']}'")
            segment_details = [{'id': d['id'], 'operating_airline': d['operating_airline'],
                                'flight_number': d['flight_number'], 'equipment': d['equipment'], 'cabin': d['cabin'],
                                'dep': d['departure_id'], 'arr': d['arrive_id'], 'baggage': d['baggage'],
        ...
        price = [{'price': d['price'], 'currency': d['currency']} for d in price]
        ...
        passenger = [{'gender': d['gender'], 'type': d['type'], 'first_name': d['first_name'],
                      'last_name': d['last_name'], 'date_of_birth': d['date_of_birth'], 'citizenship': d['citizenship'],
                      'document': d['document_id']} for d in passenger]
